...
`reason_code` 是一組固定的代碼（`unreachable`、`sso_redirect_failed`、
...
2. **斷路器。** 連續失敗達門檻後暫停登入檢查，並以指數退避（1h → 2h → 4h⋯）恢復。送出帳密後又被帶回登入表單，一律視為 SSO 轉導失敗（`sso_redirect_failed`）而非密碼錯誤 —— 監控帳號的密碼不會變，而 NTUST 在自身轉導失敗時也會把人送回表單。它照一般門檻處理，但同一次檢查絕不重送帳密。登入憑證無效或帳號不符則**立即**觸發。另有每 24 小時的嘗試上限（預設 30），必須高於登入間隔本身會產生的次數（每小時一次 = 24 次），否則服務拒絕啟動。
3. **斷路器開啟時該檢查顯示「檢查暫停」，不是「異常」。** 狀態頁不應該報告一個它沒有實際驗證過的故障。但若暫停前最後一次登入是失敗的（`paused_after_failure`），服務列仍維持「部分異常」，不會因為暫停就轉綠。
